...
- **待办事项**:
    ...
    - [x] [2026-08-19] 审查确认的 8 个问题（cleaner 并集、高度复核时序、死代码注释、宽度防护、修复超时、跨模块依赖、死变量、标题注入）全部修复。
- **风险与阻碍**:
    - **CORS 严格限制**: 即使使用了 fetch 降级，部分完全禁止跨域且无 CORS 头的图片仍可能无法加载。

## 会话连续性 (Session Continuity)

- **上次会话**: [2026-08-19] 完成全量代码审计与优化：修复 3 个正确性缺陷（Readability 破坏活页面、分页样式、XSS），性能优化（按需注入/懒加载/blob 下载），模块拆分（cleaner.js/pdf.js），测试从 6 个通过提升到 25 个全部通过。
- **当前会话**: [2026-08-19] 完成 3 个未推送提交的审查并修复全部 8 个确认问题（选择器按路径拆分、高度复核后移、图片修复超时兜底、sendProgress 兜底、标题注入防护等），测试保持全绿。
